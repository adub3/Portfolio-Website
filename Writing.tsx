                ...
                {selectedPost ? (
                            ...
                            <div className="space-y-12">
                                {selectedPost.content.map((block, idx) => {
                                    switch(block.type) {
                                        ...
                                        case 'graph': return <React.Fragment key={idx}>{block.data.type === '3d-scatter' ? <ThreeDGraph data={block.data} onExpandChange={onGraphExpand} theme={theme} /> : <InteractiveGraph data={block.data} onExpandChange={onGraphExpand} theme={theme} />}</React.Fragment>;
                                        case 'table': return (
                    ...
                    <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <div className="mb-24 opacity-0 animate-fade-in delay-500">
                            ...
                            <p className="text-xl text-theme-text/60 max-w-lg font-mono text-sm uppercase tracking-widest mt-8 border-t border-theme-border/20 pt-4">Quick dives into maths, RL, and other simulations.</p>
                        </div>
